 ...
 * game.
 */
public class OldMinimaxPlayer extends QuartoPlayer {
   ...
   * @param board
   */
  public OldMinimaxPlayer(Board board) {
    this(board, 3);
  }
   ...
   */
  public OldMinimaxPlayer(Board board, int depth) {
    super(board);
  ...
  private double maximize(MinimaxNode node, int depth, double alpha, double beta) {
    ...
    for (Piece currentPiece : piecesToCheck) {
      if (node == this.rootNode && this.bestPiece == null) {
      ...
      for (int xCoordinate = 0; xCoordinate < Board.BOARD_LENGTH; xCoordinate++) {
        for (int yCoordinate = 0; yCoordinate < Board.BOARD_LENGTH; yCoordinate++) {
          ...
          if (nextBoard != null) {
            if (node == this.rootNode && bestMove == null) {
              bestMove = nextBoard;
            }
  ...
  private Piece selectMinimaxPiece() {
    ...
    this.maximizePiece(this.rootNode, this.depth, Double.NEGATIVE_INFINITY, Double.POSITIVE_INFINITY);
    this.getBoard().takePieceForOpponent(this.bestPiece);
    return this.bestPiece;
  }

  private double maximizePiece(MinimaxNode node, int depth, double alpha, double beta) {
    //have we reached depth limit or was the game already won?
    //then evaluate how good the current state is 
    if (node.getBoard().gameWasWon() || node.getBoard().isDraw() || depth <= 0) {
      this.evaluated++;
    ...
    for (Piece currentPiece : piecesToCheck) {
      ...
      boolean loosingPiece = false;
      outerLoop:
      for (int xCoordinate = 0; xCoordinate < Board.BOARD_LENGTH; xCoordinate++) {
        for (int yCoordinate = 0; yCoordinate < Board.BOARD_LENGTH; yCoordinate++) {
          Board nextBoard = this.prepareNextBoard(node.getBoard(), currentPiece, xCoordinate, yCoordinate);
          if (nextBoard != null) {
            if (node == this.rootNode && nextBoard.gameWasWon()) {
              loosingPiece = true;
              break outerLoop;
            }
            MinimaxNode minimizeChild = new MinimaxNode(node, nextBoard);
            double result = minimizePiece(minimizeChild, depth - 1, alpha, beta);
  ...
  private double minimizePiece(MinimaxNode node, int depth, double alpha, double beta) {
    //have we reached a leaf or was the game already won?
    //then evaluate how good the current state is 
    if (node.getBoard().gameWasWon() || node.getBoard().isDraw() || depth <= 0) {
      this.evaluated++;
    ...
    for (Piece currentPiece : piecesToCheck) {
      ...
      for (int xCoordinate = 0; xCoordinate < Board.BOARD_LENGTH; xCoordinate++) {
        for (int yCoordinate = 0; yCoordinate < Board.BOARD_LENGTH; yCoordinate++) {
          ...
          if (nextBoard != null) {
            MinimaxNode maximizeChild = new MinimaxNode(node, nextBoard);
            double result = maximizePiece(maximizeChild, depth - 1, alpha, beta);
